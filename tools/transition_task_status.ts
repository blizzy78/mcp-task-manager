...
export const TransitionTaskStatusArgsSchema = z.object({
  ...
  uncertaintyAreasRemaining: z.boolean().describe(
    `Whether areas of uncertainty remain for this task.
...
export async function handleTransitionTaskStatus({
  ...
  dependsOnTasks,
  uncertaintyAreasRemaining,
  outcomeDetails,
  recommendedNextTaskID,
}: TransitionTaskStatusArgs) {
  ...
  if (!validTransition) {
    throw new Error(
      `Invalid status transition for task '${taskID}': ${oldStatus} -> ${newStatus}.${
        invalidTransitionReason ? ` ${invalidTransitionReason}` : ''
      }`
    )
  }
  ...
  if (newStatus !== 'not-started' && parentTask && parentTask.currentStatus !== 'in-progress') {
    throw new Error(
      `Task '${taskID}' can't be started: parent task '${parentTask.taskID}' must be started first. Use 'transition_task_status' tool to transition parent task's status.`
    )
  }
  ...
  if (newStatus !== 'not-started') {
    ...
    if (depTaskID) {
      throw new Error(
        `Task '${taskID}' can't be started: dependent task '${depTaskID}' is not complete. Use 'transition_task_status' tool to transition dependent task's status.`
      )
    }
  }
  ...
  return {
    content: [
...

      {
        type: 'text',
        audience: ['assistant'],
        text: `Transitioned status of task '${taskID}': ${oldStatus} -> ${newStatus}.${
          recommendedNextTaskID ? ` Recommended task to perform next: ${recommendedNextTaskID}.` : ''
        }`,
      },
    ],
