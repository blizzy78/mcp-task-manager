...
import { InitialUncertaintyAreaSchema, newTaskID, newUncertaintyAreaID, TaskIDSchema, TaskStatusSchema, UncertaintyAreaStatusSchema, } from './tasks.js';
export const CreateTaskArgsSchema = z.object({
    ...
    parentTaskID: TaskIDSchema.optional().describe('The identifier of the parent task this task belongs to, if applicable. Must be provided if this task is a subtask of another task.'),
    dependentTaskIDs: TaskIDSchema.array().describe(`A list of task identifiers this task depends on. Must be provided if this task can't be started before all of the dependent tasks are complete.`),
    uncertaintyAreas: InitialUncertaintyAreaSchema.array().describe('A detailed list of areas where there is uncertainty about the task requirements or execution.'),
});
...
export async function handleCreateTask({ title, description, goal, parentTaskID, dependentTaskIDs, uncertaintyAreas }, taskDB) {
    if (parentTaskID) {
        ...
        if (parentTask.currentStatus === 'complete') {
            throw new Error(`Invalid task: Parent task '${parentTaskID}' is already complete. Use 'transition_task_status' tool to transition parent task status to 'in-progress' first.`);
        }
    }
    ...
    const task = {
        ...
        parentTaskID,
        dependentTaskIDs,
        uncertaintyAreas: uncertaintyAreas.map((area) => ({
            ...area,
