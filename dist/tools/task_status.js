...
export const TaskStatusArgsSchema = z.object({
    ...
    assessmentId: z
        .string()
        .min(1)
        .describe(`The unique identifier of the complexity and structure assessment for this task.
Must be acquired by using the 'assess_task' tool before this task can be started.`),
    currentStatus: TaskStatusSchema.describe(`The current status of this task.
...
- All subtasks are complete, if applicable.`),
    ...
    recommendedNextTaskId: z
        ...
        .min(1)
        .optional()
        .describe('The identifier of the next recommended task to perform after this one. May only be provided if the status is complete.'),
});
...
export async function handleTaskStatus({ taskId, parentTask, assessmentId: assId, dependsOnTasks, currentStatus, recommendedNextTaskId, }) {
    if (assId !== assessmentId(taskId)) {
        throw new Error(`Invalid assessment ID '${assId}' for task '${taskId}'. Use the 'assess_task' tool to assess this task.`);
    }
    if (currentStatus !== 'not-started' && parentTask && parentTask.currentStatus !== 'in-progress') {
        throw new Error(`Task '${taskId}' can't be started unless parent task '${parentTask.taskId}' has been started. Use the 'task_status' tool to update the parent task's status.`);
    }
    if (currentStatus !== 'not-started' && dependsOnTasks?.find((dep) => dep.currentStatus !== 'complete')) {
        throw new Error(`Task '${taskId}' can't be started unless all dependent tasks are complete. Use the 'task_status' tool to update the dependent tasks' statuses.`);
    }
