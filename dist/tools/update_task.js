...
import { newUncertaintyAreaID, TaskIDSchema, UncertaintyAreaStatusSchema, UpdatingUncertaintyAreaSchema, } from './tasks.js';
export const UpdateTaskArgsSchema = z.object({
    ...
    uncertaintyAreas: UpdatingUncertaintyAreaSchema.array().describe(`A detailed list of areas where there is uncertainty about the task requirements or execution.
...
export async function handleUpdateTask({ taskID, uncertaintyAreas, dependentTaskIDs }, taskDB) {
    ...
    for (const area of uncertaintyAreas) {
        ...
        if (!area.status) {
            throw new Error(`Invalid task update: Invalid update of uncertainty area '${area.uncertaintyAreaID}': Must provide status.`);
        }
        if (area.status === 'resolved' && !area.resolution) {
            throw new Error(`Invalid task update: Invalid update of uncertainty area '${area.uncertaintyAreaID}': Must provide resolution when updating to status 'resolved'.`);
        }
    }
