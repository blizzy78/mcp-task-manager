...
export async function handleTransitionTaskStatus({ taskID, newStatus, outcomeDetails, recommendedNextTaskID }, taskDB) {
    ...
    switch (task.currentStatus) {
        ...
        case 'complete':
            switch (newStatus) {
                case 'in-progress':
                    const parentTaskNotStarted = task.parentTaskID && taskDB.get(task.parentTaskID).currentStatus !== 'in-progress';
                    if (parentTaskNotStarted) {
                        throw new Error(`Invalid status transition: Parent task '${task.parentTaskID}' must be 'in-progress' first. Use 'transition_task_status' tool to transition parent task's status.`);
                    }
                    ...
                    if (incompleteDependentTask) {
                        throw new Error(`Invalid status transition: Dependent task '${incompleteDependentTask.taskID}' must be 'complete' first. Use 'transition_task_status' tool to transition dependent task's status.`);
                    }
                    const unresolvedUncertaintyArea = task.uncertaintyAreas.find((area) => area.status !== 'resolved');
                    if (unresolvedUncertaintyArea) {
                        throw new Error(`Invalid status transition: Must resolve uncertainty area '${unresolvedUncertaintyArea.uncertaintyAreaID}' first. Use 'update_task' tool to update uncertainty areas for this task.`);
                    }
                    break;
                default:
            ...
            switch (newStatus) {
                case 'complete':
                    ...
                    if (incompleteSubtask) {
                        throw new Error(`Invalid status transition: Subtask '${incompleteSubtask.taskID}' must be 'complete' first. Use 'transition_task_status' tool to transition subtask's status.`);
                    }
                    break;
                default:
                    throw new Error(`Invalid status transition: ${task.currentStatus} -> ${newStatus}`);
