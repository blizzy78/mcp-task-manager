...
export const RegisterTaskArgsSchema = z.object({
    ...
    parentTaskId: IDSchema.optional().describe('The identifier of the parent task this task belongs to, if applicable. Must be provided if this task is a subtask of another task.'),
    ...
    dependsOnCompletedTaskIds: IDSchema.array()
        .min(1)
        .optional()
        .describe("A list of task identifiers this task depends on. Must be provided if this task can't be started before all of the dependent tasks are complete."),
});
...
export async function handleRegisterTask({ taskId, parentTaskId, dependsOnCompletedTaskIds }) {
    const res = {
        taskId,
        parentTaskId,
